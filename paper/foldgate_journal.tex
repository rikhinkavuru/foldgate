...
retained at a fixed guarantee. A pooled split across models would leak $95.1\%$ of test targets into
calibration, and even a per-model random split duplicates the roughly $10$--$11\%$ of systems with
several ligand instances; under target-grouped cross-validation the gate still holds (AF3 risk
$0.185$, $[0.168,0.203]$, $5/5$ folds) and the advantage survives with a target-cluster bootstrap
...
on this screen is the abstention rule layered over that signal, not the base enrichment. We report
...
self-scored labels are available on request and will be archived to a Zenodo record on publication.
