...
certifies a usable operating point ($43\%$ coverage at a mean-RMSD target of $1$\,\AA) where a
distribution-free bound certifies none. On a released co-folded virtual screen the pose-confidence
signal the layer is calibrated on enriches actives above docking on the post-2022 novel targets
(EF@1\% $9.3$ vs $2.0$), abstaining on unreliable poses preserves enrichment that random abstention
destroys on $75\%$ of them, and the retained poses recover $0.15$ to $0.20$ more crystal contacts
than the rejected ones. The reliability layer is training-free and every result on Runs N' Poses
runs on released predictions with no GPU; the one cross-dataset check regenerates a single withheld
confidence field.
\end{abstract}
...
released co-folded screen, abstaining on unreliable poses lifts enrichment on novel targets and
recovers more of the crystal binding contacts (Sec.~\ref{sec:screen}).
...
(AF3~\citep{af3}, Boltz-1, Boltz-1x, Chai-1, Protenix~\citep{boltz}) on Runs N' Poses~\citep{rnp},
...
model), $12{,}125$ independent target-labels, since several ligand instances of one system fail
together and pooling them as independent draws would be anti-conservative.
...
complexes at risk $0.152$ against $0.698$ on the diverse remainder, and the error it hides grows from
$0.058$ to $0.120$ through $S_3$ even as the consensus mass shrinks. The blind spot was predicted as
...
$43\%$ coverage at a $1$\,\AA\ target where a distribution-free Hoeffding bound certifies essentially
...
to $0.91$ of crystal contacts against $0.71$ to $0.76$ for the rejected ones, a gap of $0.15$ to
$0.20$ whose $90\%$ bootstrap interval excludes zero for every model. Abstention keeps the poses that
...
frozen models' released predictions and CPU alone. The one cross-dataset check regenerates a
confidence field the public release withholds, the sole GPU step. The code is public.
