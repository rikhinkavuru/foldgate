...
labels. We validate the theorem against a known conditional and on public tabular benchmarks. The
impossibility is coverage-pinned, and a per-stratum feasibility map sharpens it: the frozen score's
usable coverage decays with novelty to zero, so on the most novel strata no operating point holds
$\alpha$ at any coverage and abstention is the only certified action. We then repair what is
repairable with group-conditional calibration and a label-free worst-subpopulation (CVaR/DRO)
certificate under multiplicity control, and we report the label budget it costs. The matching
continuous story is the other half of the same principle: on the binary pose label an exact
binomial certifier is unbeatable, while on the graded RMSD loss a variance-adaptive betting bound
certifies a usable operating point ($43\%$ coverage at a mean-RMSD target of $1$\,\AA) where the
worst-case Hoeffding bound certifies none. On a released co-folded virtual screen the pose-confidence
signal the layer is calibrated on enriches actives above docking on the post-2022 novel targets
(EF@1\% $9.3$ vs $2.0$), and used as an abstention gate over a Boltz-2 affinity ranker it preserves
enrichment that random abstention destroys on $75\%$ of them. On Runs N' Poses the accepted poses
recover $0.14$ to $0.21$ more crystal contacts than the rejected ones, a non-circular quality the
$2$\,\AA\ label does not encode. The reliability layer is training-free and every result on Runs N'
Poses runs on released predictions with no GPU; the one cross-dataset check regenerates a single
withheld confidence field.
\end{abstract}
...
finite-sample guarantee holds in $0$ of $300$ resampled runs across all models. Marginal
