...
the public pre-cutoff PDB. Each axis is binned into quartiles $S_0$ (familiar) to $S_3$, with systems that
have no training analog (missing similarity, about $3\%$ of poses, AlphaFold3 $n\!\approx\!76$) forming a
fifth no-analog stratum $S_4$. The quartile edges are fixed a priori from the similarity marginal; Table~S1
...
and $0.65,0.40,0.10,0.00,0.00$ at $\alpha=0.10$. Of the $50$ model-by-stratum cells (two axes $\times$ five
models $\times$ five strata), the ten $S_0$ reference cells are feasible by construction, leaving $40$
non-reference cells. Requiring at least $20$ accepted targets to call a stratum feasible, $\mathbf{21}$ of
...
($1005$ clusters): the AF3 headline is stable ($0.72$ combined coverage, still certified, HB upper bound
...
\begin{table}[t]
\centering\small
\caption{Every headline AF3 coverage figure, mapped to its protocol ($\alpha=0.20$).}
\label{tab:covmap}
\begin{tabular}{lllcc}
\toprule
...
Conditioning removes the confound: \emph{within} the sub-$2$\,\AA\ set (AF3 $8{,}277$ correct poses across
...
Abstention is a non-certification, not a violation, and no held gate's realized risk exceeds its expected
(pre-cutoff) risk by more than $0.00$, so temporal deployment does not silently break control. This is a
...
\textbf{Limitations.} The label is a $2$\,\AA\ BiSyRMSD against a single deposited crystal pose, which
inherits the field's pose-correctness convention~\citep{posebusters} but ignores crystallographic
resolution and alternate conformations; the graded-loss betting bound (Sec.~\ref{sec:repair}, a certified
$1$\,\AA\ mean-RMSD operating point) is the partial answer that removes the hard cutoff, and a
resolution-stratified sensitivity check is deferred pending per-entry resolution metadata. The no-analog
tail is uncertifiable by every method, where abstention is the only action. The best ranker in our
screening analysis (SI), a Boltz-2 affinity head, carries no coverage guarantee: extending the framework to
certify an affinity ranker (which needs an affinity-error label) is the top open problem. All evaluation is
retrospective; a prospective screen with wet-lab confirmation is the natural next step.
