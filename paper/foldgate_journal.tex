...
$0.55$ (AF3, $90\%$ CI $[0.52,0.58]$; $0.48$ to $0.66$ across five models) against a $0.20$ target. A per-stratum feasibility
analysis shows this is not a tuning problem: of the $40$ non-reference model-by-stratum cells, $21$ admit
\emph{no} threshold on the frozen score that reaches the target at any coverage at $\alpha=0.20$, and $14$
of those survive an exact binomial lower bound, so abstention is the only certified action there. An exact
decomposition of the realized risk explains why and yields a measurable drift diagnostic that selects the
repair from a labeled probe. Group-conditional recalibration restores per-stratum control, retaining $73\%$
of AF3 poses with the finite-sample certificate holding in four of five held-out target folds under
leakage-free cross-validation, where the native gate certifies almost none, at a price we measure: to
extend one stratum outward costs on the order of $80$ in-stratum labels, and the novel tail admits no
repair by stratification alone. A public deployment-computable proxy for the stratifier tracks the
...
$S_3$, with systems that have no training analog (missing similarity, about $3\%$ of poses, AF3 $n\!\approx\!76$)
forming a fifth no-analog stratum $S_4$. The quartile edges are fixed a priori from the similarity marginal;
...
those $40$ admit no operating point at any coverage at $\alpha=0.20$, of which $\mathbf{14}$ survive an
exact binomial (Clopper-Pearson) lower bound on $R_{Q,g}>\alpha$ at every measurable coverage; at
$\alpha=0.10$ the counts are $26$ of $40$ zero-frontier and $21$ robust. Pooling the two $\alpha$ levels
($47$ zero-frontier cell-tests) leaves $\mathbf{35}$ robust across both levels. These are per-cell exact
lower bounds; we report them as a per-test robustness check rather than a family-wise-corrected
...
FEASIBLE / ABSTAIN verdict (Fig.~\ref{fig:cards}). The pattern is the paper in one glance: familiar strata
are FEASIBLE with a certified operating point, and the novel tail reads ABSTAIN, the certified action. This
turns the layer from an audit into a deliverable a practitioner reads before trusting a delivered pose.

\begin{figure}[t]
\centering
\includegraphics[width=0.98\textwidth]{../results/figures/certificate_cards.png}
\caption{Certificate cards (ligand axis, $\alpha=0.20$), one per (model, stratum). Green FEASIBLE cards
carry a certified operating point (retained coverage, accepted $n$, realized risk with its
Clopper-Pearson upper bound, reliability drift $D$); red ABSTAIN cards report that no certified operating
point exists, which is itself the certified action. The full grid across both novelty axes is in the SI.}
\label{fig:cards}
\end{figure}
...
test targets into calibration where the grouped protocol shares zero. At the stringent $\alpha=0.10$ the
combined gate certifies $13\%$ of AF3 poses at realized risk $0.066$ (pooled upper bound $0.094$).
...
Hoeffding-Bentkus upper bound. A pooled HB$^{\uparrow}\le\alpha$ (\checkmark) is a sufficient aggregate
certificate; a value $>\alpha$ means the pooled bound does not clear $\alpha$ even where most folds hold.
Chai/Protenix native gates abstain outright.}
\label{tab:loto}
