...
Each script produces one figure and its backing tables. Inputs come from released prediction sets (see `../data/DATASETS.md`); outputs land in `../results/`. Every run logs resolved config + git SHA + `pixi.lock` hash. `make repro` (or `pixi run repro`) runs the DVC pipeline end to end.
...
| `e5_generality.py` | Holds across AF3/Boltz/Chai and across pose / interface / affinity-rank. | RNP + FoldBench, all models/tasks | AURC grid by model × task |
| `e3b_weighted_repair.py` | Weighted conformal repairs coverage under shift without target labels (moderate regime); honest limit on the extreme regime. | RNP + novelty covariates + combined score | risk on target: naive vs weighted vs target-calibrated |
...
| `e9_continuous_risk.py` | Continuous-RMSD risk beyond the 2 Å convention; certified mean-RMSD gate. | RNP + combined score | mean-RMSD risk-coverage + certified continuous gate |
